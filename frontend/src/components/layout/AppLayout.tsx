      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-[#E2E8F0] bg-white px-4 lg:px-6">
          <button
            ...
            className="cursor-pointer rounded-lg p-2 text-[#64748B] hover:bg-[#F8FAFC] lg:hidden"
            aria-label="فتح القائمة"
          >
            <Menu className="h-5 w-5" />
          </button>

          <h1 className="text-lg font-bold text-[#0EA5A4] lg:hidden">الشغيلي</h1>
          <div className="hidden lg:block" />

          <div className="relative">
            <button
              onClick={() => setUserMenuOpen((open) => !open)}
              className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 transition-colors hover:bg-[#F8FAFC]"
            >
            ...
            {userMenuOpen && (
              <>
                ...
                />
                <div className="absolute left-0 top-full z-20 mt-1 w-48 rounded-lg border border-[#E2E8F0] bg-white py-1 shadow-lg">
                  <Link
                    to="/settings"
